...
For the autoregression process, we check the pacf plot and see that there are two significant peaks up to 4, and no significant peaks afterwards. it suggest 4 autoregressive terms.   
...
Now we add the manual fit and automatic fit together. 
...
We compare the result of both manual and auto fit ARIMA model and select the model with smallest AICc as the best model. Here we will select the manually fitted one "arima_013" as our best model, although the search model had almost identical performance.  
